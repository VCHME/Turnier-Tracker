...
(function(){
  function boot(){
    ...
    if(window && window.console){ console.log('VCHME TT 1.1.6 boot'); }
    ...
    function buildKO(){
      ...
      // Future rounds placeholders (slots assigned sequentially now; teams fill as winners propagate)
      var count = Math.ceil(entrantsR1.length/2); var r=2;
      while(count>=1){
        var entrants = new Array(count*2).fill(null);
        makeRound(r, entrants);
        if(count===1) break;
        count = Math.ceil(count/2); r++;
      }

      // Bronze before Final; allocate slot now (after Semis)
