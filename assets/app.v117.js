...
(function(){
  function boot(){
    ...
    if(window && window.console){ console.log('VCHME TT 1.1.7 boot'); }
    ...
    function buildKO(){
      ...
      // Order: rank1s, then rank2s, then rank3s, etc. (group order stable)
      var byRank = {};
      ...
      var seeded=[];
      Object.keys(byRank).sort(function(a,b){ return (parseInt(a,10)-parseInt(b,10)); }).forEach(function(r){ byRank[r].forEach(function(q){ seeded.push(q.team); }); });
      ...
      if(N!=target){
        var need = N - target;
        var low = teams.slice(-2*need);   // lowest ranks go to play-ins
        rest = teams.slice(0, N - 2*need);
        for(var i=0;i<need;i++){ prelim.push([low[i*2], low[i*2+1]]); }
      } else { rest=teams.slice(); }

      st.ko_matches = {}; st.ko_started=true; st._place58=false;

      var globalSlot=1;
      function assignCourt(roundName, slotIndex){
        // Semis & Final & Bronze => Court 1
        if(roundName==='Halbfinale' || roundName==='Finale' || roundName==='Spiel um Platz 3') return 1;
        // otherwise distribute across available courts
        var c = ((slotIndex-1) % st.courts) + 1;
        return c;
      }

      // Create Play-Ins (chronological first)
      ...
      function makeRound(roundNum, entrants){
        var pairs=[];
        for(var i=0;i<entrants.length;i+=2){ pairs.push([entrants[i]||null, entrants[i+1]||null]); }
        pairs.forEach(function(p,i){
          var id='KO'+roundNum+'M'+(i+1);
          var name = (roundNum===1 && pairs.length>4)? 'Achtelfinale' : (roundNum===1 && pairs.length===4)? 'Viertelfinale' : (roundNum===2 && pairs.length===2)? 'Halbfinale' : (pairs.length===1? 'Finale' : ('Runde '+roundNum));
          var ci = assignCourt(name, globalSlot);
          st.ko_matches[id]={id:id, round:roundNum, roundName:name, slot:globalSlot++, a:p[0], b:p[1], sa:null, sb:null, winner:null, loser:null, court:'Court '+ci, ci:ci};
        });
      }

      var entrantsR1 = rest.slice();
      makeRound(1, entrantsR1);

      
// Future rounds placeholders (fixed): generate correct number of matches per round.
// We'll create Semis next, then Bronze slot, then Final, etc.
      (function(){
        // Determine R1 match count
        var r1Matches = Math.ceil(entrantsR1.length/2);
        var r = 2;
        var count = r1Matches;
        while(count >= 1){
          // Build entrants placeholder of length == count, not count*2
          var entrants = new Array(count).fill(null);
          // For naming inside makeRound we rely on pairs length; OK
          makeRound(r, entrants);
          // If we just created Semis (count === 2), insert Bronze slot now
          if(count === 2){
          ...
          if(count === 1) break; // Final already created
          count = Math.ceil(count/2);
          r++;
        }
      })();

// Optional 5–8 classification

      var r1List = Object.values(st.ko_matches).filter(function(m){ return m.round===1 && m.roundName==='Viertelfinale'; });
      if(st.allPlaces && r1List.length===4){
        st._place58 = true;
        for(var c=1;c<=2;c++){
    ...
    function renderKO(){
      ...
      var next = all.filter(function(m){ return !(Number.isFinite(m.sa)&&Number.isFinite(m.sb)); }).slice().sort(function(a,b){ return a.slot-b.slot; });
