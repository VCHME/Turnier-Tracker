...
(function(){
  ...
  var courtsInfo=$('#tt-courts-info'), doneGrid=$('#tt-done-grid'), nextGrid=$('#tt-next-grid'), tablesBox=$('#tt-tables'), tablesWrap=$('#tt-tables-wrap');
  var koDoneGrid=$('#tt-ko-done-grid'), koNextGrid=$('#tt-ko-next-grid'), rankBody=$('#tt-rank-body');
  var btnToggleTables=$('#tt-toggle-tables'), btnToggleGroupsBlock=$('#tt-toggle-groups-block');
  ...
  function renderTables(){
    ...
    for(var gi=0; gi<gs.length; gi++){
      ...
      var gcls=(g==='G1'?'g1':(g==='G2'?'g2':(g==='G3'?'g3':'g4')));
      var html='<table class="vchme-tt__table"><thead><tr>'+
        '<th class="ghead '+gcls+'">'+g+'</th><th>P</th><th>W</th><th>L</th><th>PF</th><th>PA</th><th>Diff</th></tr></thead><tbody>';
  ...
  document.addEventListener('click', function(ev){
    ...
    switch(b.id){
      ...
      case 'tt-toggle-done': { ui.doneHidden=!ui.doneHidden; if(doneGrid) doneGrid.style.display=ui.doneHidden?'none':'grid'; b.textContent=ui.doneHidden?'Einblenden':'Ausblenden'; break; }
      case 'tt-toggle-ko-done': { ui.koDoneHidden=!ui.koDoneHidden; if(koDoneGrid) koDoneGrid.style.display=ui.koDoneHidden?'none':'grid'; b.textContent=ui.koDoneHidden?'Einblenden':'Ausblenden'; break; }
      case 'tt-toggle-tables': { ui.tablesHidden=!ui.tablesHidden; if(tablesWrap) tablesWrap.style.display=ui.tablesHidden?'none':'block'; b.textContent=ui.tablesHidden?'Tabellen einblenden':'Tabellen ausblenden'; break; }
      case 'tt-toggle-groups-block': { ui.groupsHidden=!ui.groupsHidden; var grpCard=gView.querySelector('.vchme-tt__card'); if(grpCard) grpCard.style.display=ui.groupsHidden?'none':'block'; b.textContent=ui.groupsHidden?'Gruppenphase einblenden':'Gruppenphase ausblenden'; break; }
    }
  }, true);
