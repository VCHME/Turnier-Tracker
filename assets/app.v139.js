...
(function(){
  ...
  var tablesBox=$('#tt-tables'), tablesWrap=$('#tt-tables-wrap'), tablesGrid=$('#tt-tables');
  ...
  // Interleaving scheduler with rest constraint (1-slot gap if possible)
  function planGroupSchedule(){
    var courts=st.courts;
    var gs=Object.keys(st.g_assign);
    // build per-group match queues sorted by round, keep round-grouping
    var queues={}, idx={}, lastPlayed={}, global=[];
    for(var gi=0; gi<gs.length; gi++){
      ...
      queues[g]=list; idx[g]=0;
    }
    var slot=1, placed=0, total=Object.keys(st.g_matches).length, safety=0;
    while(placed<total && safety<10000){
      safety++;
      var progress=false;
      // one cycle over groups
      for(var gi2=0; gi2<gs.length; gi2++){
        var gname=gs[gi2];
        var q=queues[gname]; var p=idx[gname];
        if(p>=q.length) continue;
        // try to find a match not violating rest (lookahead small window in same group)
        var pick=-1;
        for(var look=0; look<3 && (p+look)<q.length; look++){
          var cand=q[p+look];
          var a=cand.a, b=cand.b;
          var la=lastPlayed[a]||-9999, lb=lastPlayed[b]||-9999;
          if((slot-la)>=2 && (slot-lb)>=2){ pick=p+look; break; }
        }
        if(pick===-1){
          // relax: take next available
          pick=p;
        }
        var m=q[pick];
        // remove from queue at pick by swapping
        q.splice(pick,1);
        // place
        m.slot=slot;
        m.rslot=(m.rslot||0)+1;
        m.ci = ((slot-1) % courts) + 1;
        m.court='Court '+m.ci;
        lastPlayed[m.a]=slot;
        lastPlayed[m.b]=slot;
        global.push(m);
      ...
      if(!progress){
        ...
        for(var gi3=0; gi3<gs.length; gi3++){
          var g3=gs[gi3]; var q3=queues[g3];
          if(idx[g3]<q3.length){
            var mm=q3[idx[g3]]; q3.splice(idx[g3],1);
            mm.slot=slot; mm.ci=((slot-1)%courts)+1; mm.court='Court '+mm.ci; global.push(mm);
            lastPlayed[mm.a]=slot; lastPlayed[mm.b]=slot; placed++; slot++; break;
          }
        }
      }
    }
    ...
    for(var i=0;i<global.length;i++){
      var gmid=global[i].id; st.g_matches[gmid]=global[i];
    }
  }
  ...
  function renderTables(){
    ...
    var gs=Object.keys(st.g_assign);
    // set 2 columns if even number of groups
  ...
  document.addEventListener('click', function(ev){
    ...
    switch(b.id){
      ...
      case 'tt-toggle-done': {
        ui.doneHidden=!ui.doneHidden;
        if(doneGrid) doneGrid.style.display=ui.doneHidden?'none':'grid';
        b.textContent=ui.doneHidden?'Einblenden':'Ausblenden';
        break;
      }
      case 'tt-toggle-ko-done': {
        ui.koDoneHidden=!ui.koDoneHidden;
        if(koDoneGrid) koDoneGrid.style.display=ui.koDoneHidden?'none':'grid';
        b.textContent=ui.koDoneHidden?'Einblenden':'Ausblenden';
        break;
      }
      case 'tt-toggle-tables': {
        ui.tablesHidden=!ui.tablesHidden;
        var content = document.getElementById('tt-tables');
        if(content) content.style.display = ui.tablesHidden ? 'none' : 'grid';
        b.textContent=ui.tablesHidden?'Einblenden':'Ausblenden';
        break;
      }
      case 'tt-toggle-groups-block': {
        ui.groupsHidden=!ui.groupsHidden;
        var grpCard=gView.querySelector('.vchme-tt__card');
        if(grpCard) grpCard.style.display=ui.groupsHidden?'none':'block';
        b.textContent=ui.groupsHidden?'Gruppenphase einblenden':'Gruppenphase ausblenden';
        break;
      }
    }
  }, true);
